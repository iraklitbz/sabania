<script setup lang="ts">
  ...
  import Incrase from "~/components/Inputs/Incrase.vue";
  type Apartment = {
    id: number,
    name: string
  }
  ...
  const totalPrice = computed(() => {
    return calculatePrice(props.data.price.price, props.data.price.travelers, props.data.price.maxTravelers, props.data.price.increasePrice, apartments().travelers)
  })
...
<template>
  <aside class="top-32 flex w-full shrink-0 flex-col gap-3 lg:sticky lg:h-full lg:w-4/12 lg:gap-6">
    <div class="bg-white border border-solid border-gray-300 p-4 rounded-2xl shadow-xl">
        <h2 class="text-xl lg:text-2xl xl:text-3xl font-bold mb-5"> {{ currencyFormat(totalPrice) }} <span class="font-normal text-lg ml-1">night</span></h2>
        <FormKit
            type="form"
            :actions="false"
